...
describe('renderSchemas: equivalence with buildSchemas', () => {
  for (const [rel, abi] of Object.entries(FIXTURES)) {
    it(`fixture ${rel}: generated schemas match runtime parse output`, () => {
      const source = renderSchemas(abi);
...
describe('renderSchemas: header and structure', () => {
  it('emits header with tool@version', () => {
    const abi = [
      {
        type: 'function',
        name: 'foo',
    ...
    const src = renderSchemas(abi as unknown as Abi);
    expect(src).toMatch(/^\/\/ Generated by abi-to-zod@\d+\.\d+\.\d+/);
  ...
  it('is deterministic: same ABI -> identical bytes', () => {
    ...
    expect(renderSchemas(abi as unknown as Abi)).toBe(renderSchemas(abi as unknown as Abi));
  });
});
